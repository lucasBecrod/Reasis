...
# -*- coding: utf-8 -*-
"""
Consolidador Final V2 - Proyecto Reasis
Script que consolida todos los datos usando transacciones correctamente
"""
...
def procesar_competencia_digital_docentes(conn):
    ...
    try:
        ...
        print(f"   - Datos de competencia digital: {datos_procesados}")
        
    except Exception as e:
        print(f"❌ Error procesando {archivo_path.name}: {e}")
        conn.rollback()

def procesar_datos_academicos(conn):
    ...
    for archivo_relativo, materia in archivos_academicos:
        archivo_path = Path("assets/Consultoria") / archivo_relativo
        if not archivo_path.exists():
            print(f"❌ Archivo no encontrado: {archivo_path}")
            continue
        ...
        try:
            ...
            print(f"   ✅ {materia}: {instituciones_procesadas} instituciones, {datos_procesados} datos")
            total_instituciones += instituciones_procesadas
            total_datos += datos_procesados
                    
        except Exception as e:
            print(f"❌ Error procesando {archivo_path.name}: {e}")
            conn.rollback()
    
    print(f"\n✅ Procesamiento académico completado:")
    print(f"   - Total instituciones: {total_instituciones}")
    print(f"   - Total datos académicos: {total_datos}")
...
def main():
    """Función principal"""
    print("🚀 CONSOLIDADOR FINAL V2 - PROYECTO REASIS")
    print("=" * 60)
    ...
    procesar_datos_academicos(conn)
