...
# -*- coding: utf-8 -*-
"""
MOSTRAR INSTITUCIONES POR CLUSTER V3 - SIN EMOJIS
"""

import sqlite3
import pandas as pd

def mostrar_instituciones_clusters():
    """Muestra instituciones representativas por cluster sin emojis"""
    ...
    for cluster in sorted(df['cluster_asignado'].unique()):
        ...
        for idx, (_, inst) in enumerate(muestra.iterrows(), 1):
            ...
            print(f"   {nombre}")
            print(f"   Ubicacion: {inst['distrito']}, {inst['provincia']}, {inst['departamento']}")
            print(f"   Tipo: {inst['area_censo']} | {inst['nivel_educativo']} | {inst['modalidad']}")
            ...
            print(f"   Datos: Estudiantes {alumnos} | Docentes {docentes} | Secciones {secciones}")
            print()
        
        print("ESTADISTICAS DEL CLUSTER:")
        ...
        print(f"   - Distribucion area: {areas_str}")
    ...
    print(f"Departamentos: {len(df['departamento'].unique())}")
...
if __name__ == "__main__":
    mostrar_instituciones_clusters()