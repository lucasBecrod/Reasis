#!/usr/bin/env python3
"""
Evaluación Metodológica Completa - Proyecto Reasis (Versión Compatible)
Análisis de completitud de las 12 variables metodológicas
"""
...
def main():
    ...
    query_x11 = """
        ...
        WHERE total_estudiantes_2024 IS NOT NULL 
          AND total_docentes_2024 IS NOT NULL
          AND total_docentes_2024 > 0
    """
    ...
    query_x12 = "SELECT COUNT(*) as total FROM datos_toe_servicios_2024 WHERE tipo_organizacion_escolar IS NOT NULL"
    ...
    # Análisis instituciones del estudio
    print(f"\nANALISIS INSTITUCIONES DEL ESTUDIO:")
    conn = sqlite3.connect('reasis_database.db')
    ...
    df_estudio = pd.read_sql_query(query_estudio, conn)
    total_estudio = df_estudio['total'].sum()
    
    print(f"Total instituciones del estudio: {total_estudio}")
    for _, row in df_estudio.iterrows():
        print(f"  Red {row['numero_fya']}: {row['total']} instituciones")
    
    # Verificar cobertura en variables clave
    query_cobertura = """
        SELECT 
            COUNT(DISTINCT ie.codigo_modular) as total_estudio,
            COUNT(DISTINCT CASE WHEN ra.codigo_modular IS NOT NULL THEN ie.codigo_modular END) as con_ila,
            COUNT(DISTINCT CASE WHEN eib.codigo_modular IS NOT NULL THEN ie.codigo_modular END) as con_eib,
            COUNT(DISTINCT CASE WHEN toe.codigo_modular IS NOT NULL THEN ie.codigo_modular END) as con_toe
        FROM instituciones_educativas ie
        LEFT JOIN (SELECT DISTINCT codigo_modular FROM resultados_academicos) ra ON ie.codigo_modular = ra.codigo_modular
        LEFT JOIN datos_eib_minedu eib ON ie.codigo_modular = eib.codigo_modular
        LEFT JOIN datos_toe_servicios_2024 toe ON ie.codigo_modular = toe.codigo_modular
        WHERE ie.numero_fya IN ('44', '47', '48', '54', '72', '79')
    """
    
    cobertura = pd.read_sql_query(query_cobertura, conn)
    
    print(f"\nCOBERTURA EN INSTITUCIONES DEL ESTUDIO:")
    print(f"Con datos ILA: {cobertura.iloc[0]['con_ila']}/{total_estudio} ({cobertura.iloc[0]['con_ila']/total_estudio*100:.1f}%)")
    print(f"Con datos EIB: {cobertura.iloc[0]['con_eib']}/{total_estudio} ({cobertura.iloc[0]['con_eib']/total_estudio*100:.1f}%)")
    print(f"Con datos TOE: {cobertura.iloc[0]['con_toe']}/{total_estudio} ({cobertura.iloc[0]['con_toe']/total_estudio*100:.1f}%)")
